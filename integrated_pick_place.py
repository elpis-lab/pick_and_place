...
class IntegratedPickPlace(Node):
    def __init__(self):
        ...
        self.prompt = "screw_driver" #"bottle"#"banana" 
    ...
    async def process_new_pick_api(self):
        """
        Asynchronously call the place API endpoint
    ...
    async def execute_pick_place(self):
        ...
        try:
            ...
            target_pose.position.x = target_pose.position.x #- 0.07
            target_pose.position.y = target_pose.position.y #+ 0.02
            ...
            print("Current Pose Orientation: ", current_pose.orientation.z)
            print("Target Pose Orientation: ", target_pose.orientation.z)
            ...
            # Do you want to retry the pick operation
            retry_pick = input("Do you want to retry the pick operation? (y/n): ")
            if retry_pick.lower() == 'y':
                self.get_logger().info("Retrying pick operation")
                new_pick_api_result = await self.process_new_pick_api()
                print("New Pick API Result: ", new_pick_api_result)
                # Open gripper
                await self.control_gripper("open")
                if new_pick_api_result:
                    new_pick_api_result = new_pick_api_result['results'][0]
                    target_pose = Pose()
                    target_pose_list = new_pick_api_result['position_base']
                    target_pose.position.x = target_pose_list[0]
                    target_pose.position.y = target_pose_list[1]
                    target_pose.position.z =  - fused_grasp['translation'][2]  + self.gripper_offset + current_pose.position.z
                    target_pose.orientation = current_pose.orientation
                    self.get_logger().info(f"Calculated New Pick Pose: {target_pose}")
                    self.update_marker_pose(target_pose)
                    # Plan to pose
                    if self.cartesian_path:
                        trajectory = self.robot_control.plan_to_pose_cartesian(target_pose)
                        # Additonal params for plan to pose cartesian step_size, jump_threshold
                ...
                self.get_logger().info("Pick operation failed")
                temp_execute = input("Do you want to continue or exit? (y/n): ")
                if temp_execute.lower() == 'n':
                    return False
            ...
            target_pose_place.position.z = current_pose_1.position.z + 0.07
